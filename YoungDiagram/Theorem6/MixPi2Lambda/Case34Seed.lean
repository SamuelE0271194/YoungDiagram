...
/-- §17 Case 4 level-1 gap: for Label 4, `prime X` and `prime Y` lie in
`Mix (2 • Lambda, Pi)`, so their signature components are integers.  A strict
gap in *both* components at level 1 therefore gives a rank gap of at least `2`. -/
lemma case4_gap2 {N : ℕ} (X Y : nMixPi2Lambda N)
    (hseed1 :
  ...
  have hmemX : Chromosome.prime^[1] X.1.1 ∈ Mix (2 • Lambda, Pi) := by
    have h := Variety.prime_mem_Mix_Pi_2Lambda_iterate X.1.2 1
    rwa [if_neg (by decide)] at h
  have hmemY : Chromosome.prime^[1] Y.1.1 ∈ Mix (2 • Lambda, Pi) := by
    have h := Variety.prime_mem_Mix_Pi_2Lambda_iterate Y.1.2 1
    rwa [if_neg (by decide)] at h
  obtain ⟨nx, hnx⟩ := Mix2LambdaSection17.signature_Mix_2Lambda_Pi_isNat hmemX
  obtain ⟨ny, hny⟩ := Mix2LambdaSection17.signature_Mix_2Lambda_Pi_isNat hmemY
...
lemma case4_Ydrop_fst_strong_even
  ...
  have hcond7 := Mix2LambdaSection17.cond_15_7_Mix_Pi_2Lambda Y.1.2 i
...
lemma case4_Ydrop_snd_strong_even
    ...
    (Sigma.sigma Y.1.1 i).2 - (Sigma.sigma Y.1.1 (i + 2)).2 ≤
      (Sigma.sigma X.1.1 0).1 + (Sigma.sigma X.1.1 0).2 -
        ((Sigma.sigma X.1.1 1).1 + (Sigma.sigma X.1.1 1).2) - 2 := by
  have hcond6 := Mix2LambdaSection17.cond_15_6_Mix_Pi_2Lambda Y.1.2 i
...
lemma sigma_zero_eq {N : ℕ} (X Y : nMixPi2Lambda N) (hXY : X.1 < Y.1) :
...
lemma case4_seed_fst {N k : ℕ} (X Y : nMixPi2Lambda N) (hXY : X.1 < Y.1)
  ...
  have hcond7 := Mix2LambdaSection17.cond_15_7_Mix_Pi_2Lambda Y.1.2 0
...
lemma case4_seed_snd {N k : ℕ} (X Y : nMixPi2Lambda N) (hXY : X.1 < Y.1)
  ...
  have hgap2 := case4_gap2 X Y hseed1
  have hcond6 := Mix2LambdaSection17.cond_15_6_Mix_Pi_2Lambda Y.1.2 0
...
lemma case4_window_fst {N k : ℕ} (X Y : nMixPi2Lambda N) (hXY : X.1 < Y.1)
    (hr1 : (Chromosome.prime^[1] X.1.1).rank < (Chromosome.prime^[1] Y.1.1).rank)
    (hseed1 :
      (signature (Chromosome.prime^[1] X.1.1)).1 <
          (signature (Chromosome.prime^[1] Y.1.1)).1 ∧
...
lemma case4_window_snd {N k : ℕ} (X Y : nMixPi2Lambda N) (hXY : X.1 < Y.1)
    (hr1 : (Chromosome.prime^[1] X.1.1).rank < (Chromosome.prime^[1] Y.1.1).rank)
    (hseed1 :
      (signature (Chromosome.prime^[1] X.1.1)).1 <
          (signature (Chromosome.prime^[1] Y.1.1)).1 ∧
