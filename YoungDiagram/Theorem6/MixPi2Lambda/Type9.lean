...
private lemma type9_signature_eq_before
  ...
  have heq : 2 * p + 2 - j - 2 = 2 * p - j := by omega
  rw [signature_ofRank_eq₂ (k := 2 * p + 2 - j) (by omega),
    ← add_assoc, signature_ofRank_sum_even, heq]
  · simp only [Nat.cast_sub h1, Nat.cast_sub h0,
...
private lemma type9_signature_mid
    ...
    signature (Chromosome.prime^[2 * p + 1] (Y9 p hε).1) =
      signature (Chromosome.prime^[2 * p + 1] (X9 p).1) +
        signature (Gene.ofRank 1 (-ε)) := by
  simp only [X9_eq, Y9_eq, iterate_map_add, prime_iterate_ofRank]
  have h0 : 2 * p - (2 * p + 1) = 0 := by omega
  have h1 : 2 * p + 1 - (2 * p + 1) = 0 := by omega
  have h2 : 2 * p + 2 - (2 * p + 1) = 1 := by omega
  simp [h0, h2, Gene.ofRank_zero]

/-- The first reduction in §17 for Label 4. -/
lemma exists_mutation_le_type9
  ...
  have rest_mem : restval ∈ Mix (Pi, 2 • Lambda) :=
    sub_single_two_mem_Mix_Pi_2Lambda X.1.2
      (show Odd g.rank by rw [hgrank]; exact ⟨p, rfl⟩)
  let rest : Mix (Pi, 2 • Lambda) := ⟨restval, rest_mem⟩
  have hg_ofRank :
      Gene.ofRank (2 * p + 1) .NonPolarized =
  ...
  have hY_no_gene : ∀ g' : Gene, g'.rank = 2 * p + 1 → Y.1.1 g' = 0 := by
    ...
    have hg'odd : Odd g'.rank := by rw [hgr']; exact ⟨p, rfl⟩
    have hg'oddPart : 0 < Y.1.1.oddPart g' := by
      rw [oddPart_eq, Finsupp.filter_apply, if_pos hg'odd]
      exact hg'Y
    have hg'NP :=
      Mix2LambdaSection17.type_eq_nonpolarized_of_mem_twoLambda Y.1.2.2 hg'oddPart
    have hg'eq : g' = g :=
  ...
  have hYr_pred : Chromosome.prime^[2 * p] Y.1.1 ≠ 0 := by
    ...
    rw [hgrank, show 2 * p + 1 - 1 = 2 * p by omega] at hone
  ...
  change Sigma.sigma X.1.1 (2 * p + 1) ≤ Sigma.sigma Y.1.1 (2 * p + 1) at hle_r
  have hsig_ne :
      Sigma.sigma X.1.1 (2 * p + 1) ≠ Sigma.sigma Y.1.1 (2 * p + 1) := by
    intro heq
    ...
    · change (Y9 (ε := .Negative) p (by decide)).1 + restval ≤ Y.1.1
      ...
      · by_cases hj' : 2 * p + 2 ≤ j
        ...
        · have hjeq : j = 2 * p + 1 := by omega
          ...
          have heq :
              (signature (Chromosome.prime^[2 * p + 1] (X9 p).1) + (1, 0)) +
                  signature (Chromosome.prime^[2 * p + 1] restval) =
                (1, 0) + Sigma.sigma X.1.1 (2 * p + 1) := by
            change _ = (1, 0) +
          ...
          change
            (1 + (Sigma.sigma X.1.1 (2 * p + 1)).1 ≤
                (Sigma.sigma Y.1.1 (2 * p + 1)).1) ∧
              (Sigma.sigma X.1.1 (2 * p + 1)).2 ≤
                (Sigma.sigma Y.1.1 (2 * p + 1)).2
          exact ⟨by simpa [add_comm] using hboost, hle_r.2⟩
  · have hsnd : (Sigma.sigma X.1.1 (2 * p + 1)).2 <
        (Sigma.sigma Y.1.1 (2 * p + 1)).2 := by
    ...
    change (Sigma.sigma X.1.1 (2 * p + 1)).2 + 1 ≤
      (Sigma.sigma Y.1.1 (2 * p + 1)).2 at hboost
    ...
    · change (Y9 (ε := .Positive) p (by decide)).1 + restval ≤ Y.1.1
      ...
      · by_cases hj' : 2 * p + 2 ≤ j
        ...
        · have hjeq : j = 2 * p + 1 := by omega
          ...
          have heq :
              (signature (Chromosome.prime^[2 * p + 1] (X9 p).1) + (0, 1)) +
                  signature (Chromosome.prime^[2 * p + 1] restval) =
                (0, 1) + Sigma.sigma X.1.1 (2 * p + 1) := by
            change _ = (0, 1) +
              signature (Chromosome.prime^[2 * p + 1] X.1.1)
            rw [hdecomp]
            abel
          ...
          simp only [Prod.fst_add, Prod.snd_add, zero_add]
          change
            (Sigma.sigma X.1.1 (2 * p + 1)).1 ≤
                (Sigma.sigma Y.1.1 (2 * p + 1)).1 ∧
              1 + (Sigma.sigma X.1.1 (2 * p + 1)).2 ≤
                (Sigma.sigma Y.1.1 (2 * p + 1)).2
...
lemma exists_mutation_le_of_nonpolarized
    ...
    (hgNP : g.type = .NonPolarized) :
    ∃ Z : Mix (Pi, 2 • Lambda), MixPi2Lambda.Step X.1 Z ∧ Z ≤ Y.1 := by
  have hodd : Odd g.rank := by
    by_contra heven
    rw [Nat.not_odd_iff_even] at heven
    have hgeven : 0 < X.1.1.evenPart g := by
      rw [evenPart_eq, Finsupp.filter_apply, if_pos heven]
      exact hXg
    have hpol := IsPolarized_def'.mp (mem_Pi_iff.mp X.1.2.1) g
      (Finsupp.mem_support_iff.mpr hgeven.ne')
    exact hpol hgNP
  obtain ⟨p, hgrank⟩ := hodd
  have hodd' : Odd g.rank := by
    rw [hgrank]
    exact ⟨p, rfl⟩
  have hgoddPart : 0 < X.1.1.oddPart g := by
    rw [oddPart_eq, Finsupp.filter_apply, if_pos hodd']
    exact hXg
  have hXg2 :=
    Mix2LambdaSection17.two_le_coeff_of_mem_twoLambda X.1.2.2 hgoddPart
  rw [oddPart_eq, Finsupp.filter_apply, if_pos hodd'] at hXg2
