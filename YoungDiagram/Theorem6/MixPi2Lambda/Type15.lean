...
private lemma type15_diagonal_signature_eq_before
  ...
  have eA : 2 * q + 2 - j = (2 * q - j) + 2 := by omega
  have eC : 2 * q + 4 - j = (2 * q - j) + 4 := by omega
  rw [eA, eC, signature_ofRank_eq₂' (k := 2 * q - j) (ε := ε),
    signature_ofRank_eq₂' (k := 2 * q - j) (ε := -ε),
    show (2 * q - j) + 4 = ((2 * q - j) + 2) + 2 by omega,
    signature_ofRank_eq₂' (k := (2 * q - j) + 2) (ε := ε),
    signature_ofRank_eq₂' (k := 2 * q - j) (ε := ε)]
  abel_nf

private lemma type15_diagonal_signature_at_pred
    {q : ℕ} {ε : GeneType} {hε : ε ≠ .NonPolarized} :
    signature (Chromosome.prime^[2 * q + 1] (Y15 (le_refl q) hε).1) =
      signature (Gene.ofRank 1 ε) +
        signature (Chromosome.prime^[2 * q + 1] (X15 (le_refl q) hε).1) := by
  simp only [X15_eq, Y15_eq, iterate_map_add, prime_iterate_ofRank, map_add]
  have h1 : 2 * q - (2 * q + 1) = 0 := by omega
  have h2 : 2 * q + 2 - (2 * q + 1) = 1 := by omega
  have h3 : 2 * q + 4 - (2 * q + 1) = 3 := by omega
...
lemma exists_mutation_le_type15_diagonal
  ...
  have hevenε : Even gε.rank := by
    rw [hgrank]
    exact ⟨q + 1, by ring⟩
  have hevennegε : Even gnegε.rank := hrank ▸ hevenε
  have rest_mem : restval ∈ Mix (Pi, 2 • Lambda) :=
    sub_single_one_mem_Mix_Pi_2Lambda
      (sub_single_one_mem_Mix_Pi_2Lambda X.1.2 hevenε) hevennegε
  ...
  · exact (Subtype.ext hXeq :
      (X15 (le_refl q) hε : Mix (Pi, 2 • Lambda)) + rest = X.1) ▸
...
lemma type15_diagonal_gap_rank
  ...
  have hXeq : (X15 (le_refl q) hε).1 + restval = X.1.1 := by
    rw [hX15val]
    exact Mix2LambdaSection17.single_pair_add_rest hεcopy hnegεcopy hne
  have hY_no_gene : ∀ g : Gene, g.rank = 2 * q + 2 → Y.1.1 g = 0 := by
    intro g hgr
    by_contra hzero
    have hgY : 0 < Y.1.1 g := Nat.pos_of_ne_zero hzero
    have hpol : g.type ≠ .NonPolarized := by
      have hgeven : Even g.rank := by rw [hgr]; exact ⟨q + 1, by ring⟩
    ...
    | Negative =>
        ...
        | Negative =>
            ...
            rw [heq] at hgY
            omega
  have hYr_pred : Chromosome.prime^[2 * q + 1] Y.1.1 ≠ 0 := by
    intro hzero
    have hdom := le_iff_dominates.mp hXY.le (2 * q + 1)
    have hsource :
        ((1 : ℚ), (1 : ℚ)) ≤
          signature (Chromosome.prime^[2 * q + 1] X.1.1) := by
      have hdecomp :
          signature (Chromosome.prime^[2 * q + 1] X.1.1) =
            signature (Chromosome.prime^[2 * q + 1] (X15 (le_refl q) hε).1) +
              signature (Chromosome.prime^[2 * q + 1] restval) := by
        conv_lhs => rw [← hXeq]
        rw [iterate_map_add, map_add]
      have hsrc :
          ((1 : ℚ), (1 : ℚ)) ≤
        ...
        have hone : 2 * q + 2 - (2 * q + 1) = 1 := by omega
        rw [hone]
        cases ε with
  ...
  have hYr : Chromosome.prime^[2 * q + 2] Y.1.1 ≠ 0 :=
    Mix2LambdaSection17.prime_iterate_ne_zero_of_no_gene (by omega)
      hY_no_gene (by simpa only [show 2 * q + 2 - 1 = 2 * q + 1 by omega]
        using hYr_pred)
  have hle_r := le_iff_dominates.mp hXY.le (2 * q + 2)
  have hne_r :
      signature (Chromosome.prime^[2 * q + 2] X.1.1) ≠
        signature (Chromosome.prime^[2 * q + 2] Y.1.1) := by
    intro heq
    have hrank_lt := h17_1 (2 * q + 2) (by omega) hYr
  ...
  have hXr_mem := Variety.prime_mem_Mix_Pi_2Lambda_iterate X.1.2 (2 * q + 2)
  have hYr_mem := Variety.prime_mem_Mix_Pi_2Lambda_iterate Y.1.2 (2 * q + 2)
  have heven : Even (2 * q + 2) := ⟨q + 1, by ring⟩
...
`2g⁺(2q+2)+g⁻(2q+2)` uses the pair
`g⁻(2q+2)+g⁺(2q+2)` as a type15 source and leaves one `g⁺` in the rest. -/
lemma exists_mutation_le_type15_positive_of_snd_lt
  ...
  have hgap_succ := type16_succ_gap_negative X Y hXY hsnd_succ
...
lemma exists_mutation_le_type15_positive_of_snd_lt_of_pair
    ...
    (hsnd_succ :
      (signature (Chromosome.prime^[2 * q + 3] X.1.1)).2 <
        (signature (Chromosome.prime^[2 * q + 3] Y.1.1)).2) :
    ∃ Z : Mix (Pi, 2 • Lambda), MixPi2Lambda.Step X.1 Z ∧ Z ≤ Y.1 := by
  have hgneg_rank : gneg.rank = 2 * q + 2 := by
    rw [← hrank, hgpos_rank]
  have hgap_rank :=
...
lemma exists_mutation_le_type15_negative_of_fst_lt
  ...
  have hgap_succ := type16_succ_gap_positive X Y hXY hfst_succ
  apply exists_mutation_le_type15_diagonal
