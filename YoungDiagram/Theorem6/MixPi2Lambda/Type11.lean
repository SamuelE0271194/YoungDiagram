...
private lemma type11_signature_le_add_diagonal
  ...
  by_cases hj1 : j ≤ 2 * m
  · have eq1 : 2 * m + 2 - j = 2 * m - j + 2 := by omega
    have eq2 : 2 * n + 3 - j = 2 * n + 2 - j + 1 := by omega
    rw [eq1, signature_ofRank_eq₂' (2 * m - j), eq2]
    have hPN1 := type11_pos_neg_signature_sum_fst (2 * n + 2 - j)
    have hPN2 := type11_pos_neg_signature_sum_snd (2 * n + 2 - j)
    refine ⟨?_, ?_⟩
    ...
    · simp only [signature_ofRank_nonPolarized, Prod.snd_add]
      push_cast at *
      linarith
  · by_cases hj2 : j ≤ 2 * n + 2
    · have eq1 : 2 * m + 2 - j = 0 ∨ 2 * m + 2 - j = 1 := by omega
      have eq2 : 2 * m - j = 0 := by omega
      have eq3 : 2 * n + 3 - j = 2 * n + 2 - j + 1 := by omega
      ...
      · rw [heq]
        ...
        have hhalf : (((2 * n + 2 - j : ℕ) : ℚ) + 1) / 2 +
            (((2 * n + 2 - j : ℕ) : ℚ) + 1) / 2 =
            ((2 * n + 2 - j : ℕ) : ℚ) + 1 := by ring
        refine ⟨?_, ?_⟩
        · simp only [signature_ofRank_nonPolarized, Prod.fst_add]
          push_cast at *
          ...
          have hPN1' : (signature (Gene.ofRank 1 ε)).1 +
                (signature (Gene.ofRank (2 * n + 2 - j) GeneType.Positive)).1 +
                (signature (Gene.ofRank (2 * n + 2 - j) GeneType.Negative)).1 =
              (signature (Gene.ofRank 1 ε)).1 + ((2 * n + 2 - j : ℕ) : ℚ) := by
            linarith
          calc
            (((2 * n + 2 - j : ℕ) : ℚ) + 1) / 2 +
                (((2 * n + 2 - j : ℕ) : ℚ) + 1) / 2
            ...
            _ = 1 + ((signature (Gene.ofRank 1 ε)).1 +
                (signature (Gene.ofRank (2 * n + 2 - j) GeneType.Positive)).1 +
                (signature (Gene.ofRank (2 * n + 2 - j) GeneType.Negative)).1) := by
              rw [hPN1']
        · simp only [signature_ofRank_nonPolarized, Prod.snd_add]
          push_cast at *
          ...
          have hPN2' : (signature (Gene.ofRank 1 ε)).2 +
                (signature (Gene.ofRank (2 * n + 2 - j) GeneType.Positive)).2 +
                (signature (Gene.ofRank (2 * n + 2 - j) GeneType.Negative)).2 =
              (signature (Gene.ofRank 1 ε)).2 + ((2 * n + 2 - j : ℕ) : ℚ) := by
            linarith
          calc
            (((2 * n + 2 - j : ℕ) : ℚ) + 1) / 2 +
                (((2 * n + 2 - j : ℕ) : ℚ) + 1) / 2
            ...
            _ = 1 + ((signature (Gene.ofRank 1 ε)).2 +
                (signature (Gene.ofRank (2 * n + 2 - j) GeneType.Positive)).2 +
                (signature (Gene.ofRank (2 * n + 2 - j) GeneType.Negative)).2) := by
              rw [hPN2']
    · have eq1 : 2 * m + 2 - j = 0 := by omega
      have eq2 : 2 * m - j = 0 := by omega
      have eq3 : 2 * n + 2 - j = 0 := by omega
      have eq4 : 2 * n + 3 - j = 0 ∨ 2 * n + 3 - j = 1 := by omega
      rw [eq1, eq2, eq3]
...
private lemma type11_signature_eq_after
  ...
  have h1 : 2 * m + 2 - j = 0 := by omega
  have h2 : 2 * m - j = 0 := by omega
  have h3 : 2 * n + 2 - j = 0 := by omega
  have h4 : 2 * n + 3 - j = 0 := by omega
...
private lemma type11_signature_at_pred
    ...
    signature (Chromosome.prime^[2 * m + 1] (Y11 h_le hε).1) =
      signature (Gene.ofRank 1 (-ε)) +
        signature (Chromosome.prime^[2 * m + 1] (X11 h_le hε).1) := by
  simp only [X11_eq, Y11_eq, iterate_map_add, prime_iterate_ofRank, map_add]
  have hεsrc : 2 * m + 2 - (2 * m + 1) = 1 := by omega
  have hεtgt : 2 * m - (2 * m + 1) = 0 := by omega
  have hsrcpair : 2 * n + 2 - (2 * m + 1) = 2 * n + 1 - 2 * m := by omega
  have htgtpair : 2 * n + 3 - (2 * m + 1) = 2 * n + 2 - 2 * m := by omega
...
private lemma type11_signature_mid
    ...
    signature (Chromosome.prime^[j] (Y11 h_le hε).1) =
      ((1 : ℚ), (1 : ℚ)) +
        signature (Chromosome.prime^[j] (X11 h_le hε).1) := by
  simp only [X11_eq, Y11_eq, iterate_map_add, prime_iterate_ofRank, map_add]
  have hεsrc : 2 * m + 2 - j = 0 := by omega
  have hεtgt : 2 * m - j = 0 := by omega
  have hpair : 2 * n + 3 - j = (2 * n + 2 - j) + 1 := by omega
...
lemma type11_target_add_rest_le_of_diagonal_gap
  ...
  · by_cases hj_pred : j = 2 * m + 1
    · subst j
      ...
      calc
        signature (Gene.ofRank 1 (-ε)) +
              signature (Chromosome.prime^[2 * m + 1] (X11 h_le hε).1) +
    ...
    · by_cases hj : j ≤ 2 * n + 2
      · rw [type11_signature_mid h_le (by omega : 2 * m + 1 < j) hj]
        calc
          ((1 : ℚ), (1 : ℚ)) +
                signature (Chromosome.prime^[j] (X11 h_le hε).1) +
...
lemma exists_mutation_le_type11_of_decomp
  ...
  exact (Subtype.ext hXeq :
      (X11 h_le hε : Mix (Pi, 2 • Lambda)) + rest = X.1) ▸
...
lemma exists_mutation_le_type11_of_genes
  ...
  let restval : Chromosome :=
    X.1.1 - Finsupp.single gε 1 -
      Finsupp.single gpos 1 - Finsupp.single gneg 1
  have hevenε : Even gε.rank := by rw [hgε_rank]; exact ⟨m + 1, by ring⟩
  have hevenpos : Even gpos.rank := by rw [hgpos_rank]; exact ⟨n + 1, by ring⟩
  have hevenneg : Even gneg.rank := hrank ▸ hevenpos
  have rest_mem : restval ∈ Mix (Pi, 2 • Lambda) :=
    sub_single_one_mem_Mix_Pi_2Lambda
      (sub_single_one_mem_Mix_Pi_2Lambda
        (sub_single_one_mem_Mix_Pi_2Lambda X.1.2 hevenε) hevenpos)
      hevenneg
  have hgε_eq :
      Gene.ofRank (2 * m + 2) ε =
