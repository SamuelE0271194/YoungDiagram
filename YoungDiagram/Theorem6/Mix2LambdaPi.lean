...
Base cases (rank 0, 1) and the shared-gene sub-case are proved; the rank `≥ 2`
disjoint-support cases (Case 3 + Case 4, §15.10) are delegated to
`exists_mutation_le_case34`, which is still sorried.
-/
