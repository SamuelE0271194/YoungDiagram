...
/-- For `X ∈ Mix (Pi, Lambda)` with `0 < X g` and `g.type ≠ .NonPolarized`, the gene `g`
has even rank.

(Mirrors `MixLambdaPi.rank_odd_of_polarized`, with parity reversed because polarized
genes live in the even part for `Mix (Pi, Lambda)`.) -/
private lemma rank_even_of_polarized {X : Chromosome} (hX : X ∈ Mix (Pi, Lambda))
    {g : Gene} (hpol : g.type ≠ .NonPolarized) (hgX : 0 < X g) :
    Even g.rank := by
  by_contra hnot
  rw [Nat.not_even_iff_odd] at hnot
  -- Since g.rank is odd, g ∈ oddPart X.
  have hod : 0 < X.oddPart g := by
    rw [oddPart_eq, Finsupp.filter_apply, if_pos hnot]; exact hgX
  -- oddPart ∈ Lambda, so g.type = .NonPolarized
  have := IsNonPolarized_def'.mp (mem_Lambda_iff.mp hX.2) g
    (Finsupp.mem_support_iff.mpr (Nat.pos_iff_ne_zero.mp hod))
  exact hpol this

/-- The polarized parts of two genes in `X ∈ Mix (Pi, Lambda)` with equal rank have
even rank (= `2 * m + 2`). -/
private lemma rank_eq_two_mul_succ_succ_of_pn {gpos gneg : Gene}
    (hgpos : gpos.type = .Positive) (hgneg : gneg.type = .Negative)
    (hrank : gpos.rank = gneg.rank) {X : Chromosome}
    (hX : X ∈ Mix (Pi, Lambda)) (hXgpos : 0 < X gpos) (hXgneg : 0 < X gneg) :
    ∃ m : ℕ, gpos.rank = 2 * m + 2 := by
  have hev : Even gpos.rank :=
    rank_even_of_polarized hX (by rw [hgpos]; decide) hXgpos
  rcases hev with ⟨m, hm⟩
  -- gpos.rank ≥ 1 (rank_pos) and even, so ≥ 2.
  have hpos : 1 ≤ gpos.rank := gpos.rank_pos
  refine ⟨m - 1, ?_⟩
  omega
...
/-- For `X ∈ Mix (Pi, Lambda)`, `prime^[j] X` at *even* `j` lies in `Mix (Pi, Lambda)`,
so its signature has equal components (because in `Mix (Pi, Lambda)`, the even part
contains only even-rank genes and the odd part is `Lambda`).
...
private lemma signature_prime_iterate_even_eq_components
  ...
  have hmem : Chromosome.prime^[j] X ∈ Mix (Pi, Lambda) := by
    have := Variety.prime_mem_Mix_Pi_Lambda_iterate hX j
...
/-- Signature analog of type-7 equality before reaching rank `r = 2m + 2`. -/
private lemma signature_type7_eq_before {ε : GeneType} (hε : ε ≠ .NonPolarized)
    {m j : ℕ} (hj : j < 2 * m + 2) :
    signature (Chromosome.prime^[j] (Y7 (n := m) (le_refl m)).1) =
  ...
  rw [X7_eq, Y7_eq, iterate_map_add, iterate_map_add,
    ...
    map_add, map_add]
  -- prime^[j] X7 = ofRank(2m+2-j)ε + ofRank(2m+2-j)(-ε)
  -- prime^[j] Y7 = ofRank(2m+1-j)NP + ofRank(2m+3-j)NP
  -- Apply signature_ofRank_succ_add_pred_neg with m_arg = 2m+1-j, n_arg = 2m+3-j.
  have eq1 : 2 * m + 2 - j = 2 * m + 1 - j + 1 := by omega
  have eq2 : 2 * m + 2 - j = 2 * m + 3 - j - 1 := by omega
  have hn : 1 ≤ 2 * m + 3 - j := by omega
  have heven : Even (2 * m + 1 - j + (2 * m + 3 - j)) := by
    rw [Nat.even_iff]; omega
  have h := signature_ofRank_succ_add_pred_neg (m := 2 * m + 1 - j) (n := 2 * m + 3 - j)
    (ε := ε) hn heven
  rw [← eq1, ← eq2] at h
  exact h.symm

/-- At iterate `j = 2m+2 = r`, the type-7 source has zero signature. -/
private lemma signature_type7_source_self_eq_zero {ε : GeneType}
    (hε : ε ≠ .NonPolarized) {m : ℕ} :
    signature (Chromosome.prime^[2 * m + 2] (X7 (le_refl m) hε).1) = 0 := by
  rw [X7_eq, iterate_map_add, prime_iterate_ofRank, prime_iterate_ofRank]
  simp [Nat.sub_self, Gene.ofRank_zero]

/-- At iterate `j = 2m+2 = r`, the type-7 target has signature `(1/2, 1/2)`. -/
private lemma signature_type7_target_self_eq_half {m : ℕ} :
    signature (Chromosome.prime^[2 * m + 2] (Y7 (n := m) (le_refl m)).1) =
      ((1 : ℚ) / 2, (1 : ℚ) / 2) := by
  rw [Y7_eq, iterate_map_add, prime_iterate_ofRank, prime_iterate_ofRank,
    show 2 * m + 1 - (2 * m + 2) = 0 from by omega,
    show 2 * m + 3 - (2 * m + 2) = 1 from by omega]
  simp [Gene.ofRank_zero, signature_ofRank_nonPolarized]

/-- For `j > r = 2m+2`, the type-7 source has zero signature. -/
private lemma signature_type7_source_after_eq_zero {ε : GeneType}
    (hε : ε ≠ .NonPolarized) {m j : ℕ} (hj : 2 * m + 2 < j) :
    signature (Chromosome.prime^[j] (X7 (le_refl m) hε).1) = 0 := by
  rw [X7_eq, iterate_map_add, prime_iterate_ofRank, prime_iterate_ofRank,
    show 2 * m + 2 - j = 0 from by omega]
  simp [Gene.ofRank_zero]

/-- For `j > r = 2m+2`, the type-7 target has zero signature. -/
...
`Y.evenPart ∈ Pi` so any rank-`r` gene in `Y` (with `r` even) must be polarized). -/
private lemma Y_no_gene_of_rank_mix {X Y : Chromosome}
    ...
    {r : ℕ} (hrank_pos : gpos.rank = r) (heven : Even r)
    (g : Gene) (hgr : g.rank = r) : Y g = 0 := by
  by_contra hne
  have hgY : 0 < Y g := Nat.pos_of_ne_zero hne
  -- g has even rank, so g ∈ Y.evenPart ∈ Pi, so g.type ≠ NonPolarized.
  have hgY_even : 0 < Y.evenPart g := by
    rw [evenPart_eq, Finsupp.filter_apply, if_pos (hgr ▸ heven)]; exact hgY
  have hg_pol : g.type ≠ .NonPolarized :=
    IsPolarized_def'.mp (mem_Pi_iff.mp hYmem.1) g
...
/-- Helper: for `X ∈ Mix (Pi, Lambda)` at even r, 2 * signature(prime^[r] X).1 is a natural
number. This relies on the fact that 2 * (sig.1 + sig.2) = 2 * rank ∈ ℕ and sig.1 = sig.2. -/
private lemma two_signature_fst_isNat_of_even_iterate
    {X : Chromosome} (hX : X ∈ Mix (Pi, Lambda)) {r : ℕ} (heven : Even r) :
...
private lemma half_le_sigma_diff_at_r {X Y : Chromosome}
  ...
  have hXeq := signature_prime_iterate_even_eq_components hX heven
  have hYeq := signature_prime_iterate_even_eq_components hY heven
  set sX := signature (Chromosome.prime^[r] X) with hsX
  set sY := signature (Chromosome.prime^[r] Y) with hsY
  obtain ⟨nX, hnX⟩ := two_signature_fst_isNat_of_even_iterate hX heven
  obtain ⟨nY, hnY⟩ := two_signature_fst_isNat_of_even_iterate hY heven
...
private lemma exists_mutation_le_disjoint_pair {m : ℕ}
  ...
  -- gpos.rank is even ≥ 2, so write it as 2 * mr + 2.
  have heven_gpos : Even gpos.rank :=
    rank_even_of_polarized X.1.2 (by rw [hgpos]; decide) hXgpos
  obtain ⟨mr, hmr⟩ : ∃ mr : ℕ, gpos.rank = 2 * mr + 2 :=
    rank_eq_two_mul_succ_succ_of_pn hgpos hgneg hrank X.1.2 hXgpos hXgneg
  let r := gpos.rank
  have hr : 1 ≤ r := gpos.rank_pos
  have hr_eq : r = 2 * mr + 2 := hmr
  have hr_even : Even r := heven_gpos
  ...
  let X7' : Mix (Pi, Lambda) := X7 (m := mr) (n := mr) (le_refl mr) hε
  let Y7' : Mix (Pi, Lambda) := Y7 (m := mr) (n := mr) (le_refl mr)
  ...
  refine ⟨Z, (Subtype.ext hX_eq : X7' + rest_M = X.1) ▸
    MixPiLambda.Step.mk X7' Y7' rest_M
      (MixPiLambda.Primitive.type7 ε hε (le_refl mr)), ?_⟩
  ...
  · -- j < r: signature(prime^[j] X7) = signature(prime^[j] Y7).
    have hY7X7 : signature (Chromosome.prime^[j] Y7'.1) =
      ...
      have := signature_type7_eq_before (m := mr) hε (show j < 2 * mr + 2 from hr_eq ▸ hjr)
      exact this
    rw [hY7X7, ← hdecomp]
    exact hXYj
  · -- j = r: signature(prime^[r] X7) = 0, signature(prime^[r] Y7) = (1/2, 1/2).
    ...
    apply half_le_sigma_diff_at_r X.1.2 Y.1.2 hr_even hXYj hsig_ne'
