...
-- The odd-gap proof reuses both parity-specific window chains and needs extra elaboration budget.
set_option linter.unusedVariables false in
lemma exists_mutation_le_case4b_oddGap
  ...
  · exact exists_mutation_le_case4b_evenGap_of_sigma_window X Y hXg₁ hg₁_ge2 hg₁_one
  ...
  · exact exists_mutation_le_case4b_evenGap_of_sigma_window X Y hXg₁ hg₁_ge2 hg₁_one
      hg₂pos hg₂rank hε₂ <| by
      have h_g1_rank_odd : ¬Even g₁.rank := h_g1_rank_even
      intro hε hle hm j
      have hXYj : sigma X.1.val j ≤ sigma Y.1.val j :=
        le_iff_dominates.mp hXY.le j
      obtain ⟨hcase1, hcase2, hcase3⟩ :=
        ...
        · push Not at hjl hjr
          ...
          have hg₂_pos : g₂.type = .Positive := by
            ...
            have hg₂_ne_neg : g₂.type ≠ .Negative := by
              simpa [hε_pos, GeneType.neg_positive] using hε₂
            cases ht : g₂.type with
            | Positive => rfl
            ...
            have hb0_bi : ∀ j, 1 ≤ j → j ≤ g₁.rank + 1 →
              ...
              · have no_neg_gene_rank_g : ∀ g' ∈ X.1.val.support,
                  ...
                  have hg'_ne_neg : g'.type ≠ .Negative := by
                    intro hg'_neg
                    apply hXpn
                    exact ⟨g₁, g', hg'_rank.symm, hε_pos, hg'_neg, hXg₁pos,
                      Nat.pos_of_ne_zero (Finsupp.mem_support_iff.mp hg'_supp)⟩
                  cases ht' : g'.type with
                  | Positive => rfl
                  | Negative => exact absurd ht' hg'_ne_neg
                ...
                have h := Sigma.b0_bi_eq_a1_ai1 X.1.val X.1.2 (j - 1)
                  (fun g hg_supp hrank_le => by
                    have hg_rank_eq : g.rank = g₁.rank := by
          ...
          have hdi_sub_le_bi_sub : ∀ j, g₁.rank - 1 ≤ j → j ≤ g₂.rank - 1 →
            ...
            · have hdj_le_c01 :
              ...
              have ha01_sub1_eq_am_sub1 :
                ...
                have hg₁_altType : g₁.type = Sigma.altType g₁.rank GeneType.Positive := by
                  rw [Sigma.altType_odd g₁.rank h_g1_rank_odd]
                  exact hε_pos
                have h := x_side_equalities
