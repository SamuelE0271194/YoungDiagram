...
/-!
# §17 window-propagation core for `Mix (2 • Lambda, Pi)` no-equal-rank-pair Case 1.
...
The polarized source `X` here lies in `Pi` (every gene polarized, odd rank), so
the X-side drop is a clean gene count, while the Y-side drop is bounded by the
zeroth drop via the `Mix (2 • Lambda, Pi)` conditions (15.6)/(15.7).
-/
...
/-- Single-step rank-drop antitonicity for `Mix (2 • Lambda, Pi)`: summing
(15.6) and (15.7) shows the rank-drop `r_i - r_{i+1}` is non-increasing. -/
private lemma rank_drop_step {Z : Chromosome} (hZ : Z ∈ Mix (2 • Lambda, Pi)) (i : ℕ) :
  ...
  have h6 := cond_15_6_Mix_2Lambda_Pi hZ i
  have h7 := cond_15_7_Mix_2Lambda_Pi hZ i
  by_cases hi : Even i
...
lemma rank_drop_le {Z : Chromosome} (hZ : Z ∈ Mix (2 • Lambda, Pi)) (i : ℕ) :
    (Sigma.sigma Z i).1 + (Sigma.sigma Z i).2 -
        ((Sigma.sigma Z (i + 1)).1 + (Sigma.sigma Z (i + 1)).2) ≤
...
/-- KEY_Y for Label 3: the `Y`-side `a`-component 2-step drop at an even level is
...
lemma KEY_Y_fst {N : ℕ} (X Y : nMix2LambdaPi N)
  ...
  -- step a: c_{i+1} − c_{i+2} ≤ d_i − d_{i+1} (Label-3 (15.7), even branch)
  have hcond7 := cond_15_7_Mix_2Lambda_Pi Y.1.2 i
...
lemma KEY_Y_snd {N : ℕ} (X Y : nMix2LambdaPi N)
    (hr1 : (Chromosome.prime^[1] X.1.1).rank < (Chromosome.prime^[1] Y.1.1).rank)
    {i : ℕ} (hi : Even i) :
    (Sigma.sigma Y.1.1 i).2 - (Sigma.sigma Y.1.1 (i + 2)).2 ≤
      (Sigma.sigma X.1.1 0).1 + (Sigma.sigma X.1.1 0).2 -
        ((Sigma.sigma X.1.1 1).1 + (Sigma.sigma X.1.1 1).2) - 1 := by
  -- step b: d_{i+1} − d_{i+2} ≤ c_i − c_{i+1} (Label-3 (15.6), even branch)
  have hcond6 := cond_15_6_Mix_2Lambda_Pi Y.1.2 i
...
private lemma shift_ge {N : ℕ} (X : nMix2LambdaPi N) (gm : Gene)
...
private lemma cells_of_X {N : ℕ} (X : nMix2LambdaPi N) (gm : Gene)
    (hgm1 : X.1.1 gm = 1) :
...
lemma KEY_X_fst {N : ℕ} (X : nMix2LambdaPi N) {m k : ℕ}
...
lemma KEY_X_snd {N : ℕ} (X : nMix2LambdaPi N) {m k : ℕ}
    {gm : Gene} (hgm_rank : gm.rank = m) (hgm1 : X.1.1 gm = 1)
    (h2nd : ∀ g ∈ (X.1.1 - Finsupp.single gm 1).support, k ≤ g.rank)
    {i : ℕ} (hi1 : m ≤ i) (hi2 : i + 2 ≤ k) :
...
private lemma sigma_split {N : ℕ} (X : nMix2LambdaPi N) (gm : Gene)
    (hgm1 : X.1.1 gm = 1) (i : ℕ) :
    Sigma.sigma X.1.1 i =
...
lemma KEY_X_fst_ge {N : ℕ} (X : nMix2LambdaPi N) {k : ℕ}
...
lemma KEY_X_snd_ge {N : ℕ} (X : nMix2LambdaPi N) {k : ℕ}
    {gm : Gene} (hgm1 : X.1.1 gm = 1)
    (h2nd : ∀ g ∈ (X.1.1 - Finsupp.single gm 1).support, k ≤ g.rank)
    {i : ℕ} (hi2 : i + 2 ≤ k) :
    (Sigma.sigma X.1.1 0).1 + (Sigma.sigma X.1.1 0).2 -
...
lemma KEY_X_full_fst {N : ℕ} (X : nMix2LambdaPi N) {m : ℕ}
...
lemma KEY_X_full_snd {N : ℕ} (X : nMix2LambdaPi N) {m : ℕ}
    (hmin : ∀ g ∈ X.1.1.support, m ≤ g.rank)
    {i : ℕ} (hi2 : i + 2 ≤ m) :
    (Sigma.sigma X.1.1 i).2 - (Sigma.sigma X.1.1 (i + 2)).2 =
      (Sigma.sigma X.1.1 0).1 + (Sigma.sigma X.1.1 0).2 -
...
lemma window_even_fst_lt {N : ℕ} (X Y : nMix2LambdaPi N)
    (hr1 : (Chromosome.prime^[1] X.1.1).rank < (Chromosome.prime^[1] Y.1.1).rank)
...
lemma window_even_snd_lt {N : ℕ} (X Y : nMix2LambdaPi N)
    (hr1 : (Chromosome.prime^[1] X.1.1).rank < (Chromosome.prime^[1] Y.1.1).rank)
    {k : ℕ} {gm : Gene} (hgm1 : X.1.1 gm = 1)
    (h2nd : ∀ g ∈ (X.1.1 - Finsupp.single gm 1).support, k ≤ g.rank)
    (j0 d : ℕ) (hj0_even : Even j0) (hwin : j0 + 2 * d ≤ k)
...
lemma seed_fst_lt {N : ℕ} (X Y : nMix2LambdaPi N)
    (hr1 : (Chromosome.prime^[1] X.1.1).rank < (Chromosome.prime^[1] Y.1.1).rank)
    {i : ℕ} (hi : Even i)
...
lemma seed_snd_lt {N : ℕ} (X Y : nMix2LambdaPi N)
    (hr1 : (Chromosome.prime^[1] X.1.1).rank < (Chromosome.prime^[1] Y.1.1).rank)
    {i : ℕ} (hi : Even i)
