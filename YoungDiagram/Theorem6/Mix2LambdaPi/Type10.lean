...
lemma exists_mutation_le_type10_of_decomp
  ...
  exact (Subtype.ext hXeq :
      (X10 h_le hε hε' : Mix (2 • Lambda, Pi)) + rest = X.1) ▸
...
`g^ε(2m+3)+g^{ε'}(2n+3)+rest` and the rest membership; the caller still supplies
...
lemma exists_mutation_le_type10_of_genes
    ...
    (hg1_rank : g1.rank = 2 * m + 3) (hg2_rank : g2.rank = 2 * n + 3)
  ...
  have hodd1 : Odd g1.rank := by rw [hg1_rank]; exact ⟨m + 1, by ring⟩
  have hodd2 : Odd g2.rank := by rw [hg2_rank]; exact ⟨n + 1, by ring⟩
  have rest_mem : restval ∈ Mix (2 • Lambda, Pi) :=
    sub_single_one_mem_Mix_2Lambda_Pi
      (sub_single_one_mem_Mix_2Lambda_Pi X.1.2 hodd1) hodd2
  have hg1_eq :
      Gene.ofRank (2 * m + 3) ε = (Finsupp.single g1 1 : Chromosome) := by
    have h := Gene.ofRank_eq_gene (g := g1)
    rwa [hg1_rank, hg1] at h
  have hg2_eq :
      Gene.ofRank (2 * n + 3) ε' = (Finsupp.single g2 1 : Chromosome) := by
...
`2 g^ε(2a+3) → g^ε(2a+1) + g^ε(2a+5)`.  This is the `X ⊃ 2g⁺(m)` sub-case of
...
lemma exists_mutation_le_type10_of_double
    ...
    (X Y : nMix2LambdaPi N) (g : Gene)
    (hg : g.type = ε) (hg_rank : g.rank = 2 * a + 3)
    (hg2 : 2 ≤ X.1.1 g)
    (hZle :
  ...
  have hodd : Odd g.rank := by rw [hg_rank]; exact ⟨a + 1, by ring⟩
  have rest_mem : restval ∈ Mix (2 • Lambda, Pi) :=
    sub_single_one_mem_Mix_2Lambda_Pi
      (sub_single_one_mem_Mix_2Lambda_Pi X.1.2 hodd) hodd
  have hg_eq :
      Gene.ofRank (2 * a + 3) ε = (Finsupp.single g 1 : Chromosome) := by
...
`0` for `j ≤ 2m+1` (the two `(1,1)` increments cancel); `(1,1)` throughout the
middle window `2m+3 ≤ j ≤ 2n+3` (only the up-moving `ε'`-gene contributes); and
`0` again for `j ≥ 2n+5`.  The two transition levels `j = 2m+2, 2n+4` are handled
...
private lemma type10_signature_eq_after {m n j : ℕ} {ε ε' : GeneType}
  ...
  have e1 : 2 * m + 1 - j = 0 := by omega
  have e2 : 2 * m + 3 - j = 0 := by omega
  have e3 : 2 * n + 3 - j = 0 := by omega
  have e4 : 2 * n + 5 - j = 0 := by omega
  simp [e1, e2, e3, e4]

/-- Transition level `j = 2m+2` (paper's `m-1`): the delta is `(1,1) - s(1,ε)`,
...
private lemma type10_signature_pred {m n : ℕ} {ε ε' : GeneType}
    ...
    signature (Gene.ofRank 1 ε) +
        signature (Chromosome.prime^[2 * m + 2] (Y10 h_le hε hε').1) =
      ((1 : ℚ), (1 : ℚ)) +
        signature (Chromosome.prime^[2 * m + 2] (X10 h_le hε hε').1) := by
  simp only [X10_eq, Y10_eq, iterate_map_add, prime_iterate_ofRank, map_add]
  have e1 : 2 * m + 1 - (2 * m + 2) = 0 := by omega
  have e2 : 2 * m + 3 - (2 * m + 2) = 1 := by omega
  have eC : 2 * n + 5 - (2 * m + 2) = (2 * n + 3 - (2 * m + 2)) + 2 := by omega
  rw [e1, e2, eC, Gene.ofRank_zero, map_zero, signature_ofRank_eq₂']
  abel

/-- Transition level `j = 2n+4` (paper's `k+1`): the delta is `s(1,ε')`. -/
private lemma type10_signature_succ {m n : ℕ} {ε ε' : GeneType}
    (hε : ε ≠ .NonPolarized) (hε' : ε' ≠ .NonPolarized) (h_le : m ≤ n) :
    signature (Chromosome.prime^[2 * n + 4] (Y10 h_le hε hε').1) =
      signature (Gene.ofRank 1 ε') +
        signature (Chromosome.prime^[2 * n + 4] (X10 h_le hε hε').1) := by
  simp only [X10_eq, Y10_eq, iterate_map_add, prime_iterate_ofRank, map_add]
  have e1 : 2 * m + 1 - (2 * n + 4) = 0 := by omega
  have e2 : 2 * m + 3 - (2 * n + 4) = 0 := by omega
  have e3 : 2 * n + 3 - (2 * n + 4) = 0 := by omega
  have e4 : 2 * n + 5 - (2 * n + 4) = 1 := by omega
  rw [e1, e2, e3, e4]
...
lemma type10_target_add_rest_le_of_diagonal_gap
    ...
    (h_le : m ≤ n) (X Y : nMix2LambdaPi N) (hXY : X.1 < Y.1) (restval : Chromosome)
    (hXeq : (X10 h_le hε hε').1 + restval = X.1.1)
    (hgap_pred :
      ((1 : ℚ), (1 : ℚ)) + signature (Chromosome.prime^[2 * m + 2] X.1.1) ≤
        signature (Gene.ofRank 1 ε) +
          signature (Chromosome.prime^[2 * m + 2] Y.1.1))
    (hgap_mid : ∀ j, 2 * m + 3 ≤ j → j ≤ 2 * n + 3 →
  ...
  · by_cases hj_pred : j = 2 * m + 2
    · subst j
      ...
      calc
        ...
        _ = (((1 : ℚ), (1 : ℚ)) +
                signature (Chromosome.prime^[2 * m + 2] (X10 h_le hε hε').1)) +
              signature (Chromosome.prime^[2 * m + 2] restval) := by
              rw [type10_signature_pred]
        _ = ((1 : ℚ), (1 : ℚ)) +
              signature (Chromosome.prime^[2 * m + 2] X.1.1) := by
              rw [hdecomp]; abel
        _ ≤ signature (Gene.ofRank 1 ε) +
              signature (Chromosome.prime^[2 * m + 2] Y.1.1) := hgap_pred
    · by_cases hj_mid : j ≤ 2 * n + 3
      · rw [type10_signature_mid hε hε' h_le (by omega : 2 * m + 3 ≤ j) hj_mid]
        calc
          signature (Chromosome.prime^[j] (X10 h_le hε hε').1) +
                ((1 : ℚ), (1 : ℚ)) +
      ...
      · by_cases hj_succ : j = 2 * n + 4
        · subst j
          rw [type10_signature_succ hε hε' h_le]
          calc
            signature (Gene.ofRank 1 ε') +
                  signature (Chromosome.prime^[2 * n + 4] (X10 h_le hε hε').1) +
                signature (Chromosome.prime^[2 * n + 4] restval)
                = signature (Gene.ofRank 1 ε') +
                  (signature (Chromosome.prime^[2 * n + 4] (X10 h_le hε hε').1) +
                    signature (Chromosome.prime^[2 * n + 4] restval)) := by abel
            _ = signature (Gene.ofRank 1 ε') +
                  signature (Chromosome.prime^[2 * n + 4] X.1.1) := by rw [← hdecomp]
            _ ≤ signature (Chromosome.prime^[2 * n + 4] Y.1.1) := hgap_succ
        · have hj_after : 2 * n + 5 ≤ j := by omega
