...
lemma exists_mutation_le_case2
  ...
  have hb₀_gt_a₁ : (Sigma.sigma X.1 1).1 < (Sigma.sigma X.1 0).2 := by
    have hd₀_ge_c₁ : (Sigma.sigma Y.1 1).1 ≤ (Sigma.sigma Y.1 0).2 := by
      simpa using Sigma.cond_15_5 Y.1.val 0
    linarith [hb₀_eq_d₀]
  have hg₂_neg : ∃ g₂ : Gene,
      g₂.type = .Negative ∧
      0 < X.1.val g₂ ∧
  ...
  -- g₁ has type Positive: not Negative (from hε₁), not NonPolarized (by polarization).
  have hg₁_type : g₁.type = .Positive := by
    ...
    | Negative => exact absurd (by rw [ht, hg₁_one]; simp) hε₁
