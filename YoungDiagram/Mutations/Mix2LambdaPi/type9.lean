...
-- Φ = (2 • Λ, Π): g ranks even, g^ε ranks odd. Equation (8.9):
-- 2 g(m) → g^ε(m-1) + g^{-ε}(m+1) with m = 2k+2 even.
...
lemma mutation_type9_ne : type9X ≠ type9Y := by
  intro h
  replace h := congr_arg (· ⟨2 * k + 3, - ε, by omega⟩) h
  have h1 : 2 * k + 2 ≠ 0 := by omega
  have h2 : 2 * k + 1 ≠ 0 := by omega
  have h3 : 2 * k + 3 ≠ 0 := by omega
  simp only [Gene.ofRank_def, h1, h2, h3, ↓reduceDIte, Finsupp.coe_add, Pi.add_apply,
    Finsupp.single_apply, Gene.mk.injEq, Nat.reduceEqDiff, Nat.add_left_cancel_iff,
    false_and] at h
  split_ifs at h
  all_goals omega

lemma mutation_type9_iterate_signature_eq (i j : ℕ) (hi : i ≤ j) :
    (prime^[i] (Gene.ofRank (2 * k + 2 + j) .NonPolarized +
      Gene.ofRank (2 * k + 2 + j) .NonPolarized)).signature =
    (prime^[i] (Gene.ofRank (2 * k + 1 + j) ε +
      Gene.ofRank (2 * k + 3 + j) (- ε))).signature := by
  have le1 : i ≤ 2 * k + 1 + j := by omega
  have le2 : i ≤ 2 * k + 2 + j := by omega
  have eq1 : 2 * k + 3 + j - i - 2 = 2 * k + 1 + j - i := by omega
  simp only [iterate_map_add, prime_iterate_ofRank, map_add,
    signature_ofRank_nonPolarized]
  rw [signature_ofRank_eq₂ (k := 2 * k + 3 + j - i) (by omega), ← add_assoc,
...
lemma mutation_type9_le : type9X ≤ type9Y := by
  ...
  by_cases hle1 : i ≤ 2 * k + 1
  · have le1 : i ≤ 2 * k + 2 := by omega
    have le2 : i ≤ 2 * k + 3 := by omega
    ...
    · grind only [= Nat.even_iff]
  · by_cases hle2 : i ≤ 2 * k + 3
    · have eq1 : 2 * k + 2 - i = 0 := by omega
      have eq2 : 2 * k + 1 - i = 0 := by omega
      rw [eq1, eq2, Nat.cast_zero, zero_div, Gene.ofRank_zero, map_zero,
        ← Prod.zero_eq_mk, zero_add, zero_add]
      by_cases heq1 : i = 2 * k + 3
      · simp only [heq1, le_refl, tsub_self, Gene.ofRank_zero, map_zero]
      · have heq2 : i = 2 * k + 2 := by omega
        rw [heq2]
        have heq3 : 2 * k + 3 - (2 * k + 2) = 1 := by omega
        rw [heq3]
        match ε with
        | .NonPolarized =>
...
noncomputable def X9 : Mix (2 • Lambda, Pi) := by
  ...
  refine ⟨Gene.ofRank (2 * k + 2) GeneType.NonPolarized, ?_, ?_⟩
