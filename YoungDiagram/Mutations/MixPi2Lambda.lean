...
inductive Step : Mix (Pi, 2 • Lambda) → Mix (Pi, 2 • Lambda) → Prop
  | mk (X Y Z : Mix (Pi, 2 • Lambda)) (h : Primitive X Y) :
      Step (X + Z) (Y + Z)

lemma Primitive.isMutation {X Y : Mix (Pi, 2 • Lambda)} (h : Primitive X Y) :
    IsMutation X Y := by
  cases h with
  | type9 ε hε k =>
...
lemma Primitive.neg {X Y : Mix (Pi, 2 • Lambda)} (h : Primitive X Y) :
    Primitive (- X) (- Y) := by
  cases h with
  | type9 ε hε k =>
...
lemma Step.isMutation {X Y : Mix (Pi, 2 • Lambda)} (h : Step X Y) :
    IsMutation X Y := by
  cases h with
  | mk X Y Z h =>
    exact .add_right _ (Primitive.isMutation h)

lemma Step.neg {X Y : Mix (Pi, 2 • Lambda)} (h : Step X Y) : Step (- X) (- Y) := by
  cases h with
  | mk X Y Z hPrime =>
    rw [Mix.Pi_2Lambda_neg_add, Mix.Pi_2Lambda_neg_add]
    exact Step.mk (- X) (- Y) (- Z) hPrime.neg

lemma Step.add_right (W : Mix (Pi, 2 • Lambda)) {A B : Mix (Pi, 2 • Lambda)}
...
lemma Step.of_neg {X Y : Mix (Pi, 2 • Lambda)} (h : Step (-X) (-Y)) : Step X Y := by
