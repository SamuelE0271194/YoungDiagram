...
\begin{definition}[Variety $\Pi$]
  ...
  The variety $\Pi$ is the filtered variety of all polarized chromosomes.
\end{definition}
...
\begin{lemma}[Polarized signature is natural]
  ...
  If $X \in \Pi$, the signature components of $X$ are natural numbers.
\end{lemma}
...
\begin{lemma}[Sigma sequence determines element of $\Pi$]
  ...
  If $X, Y \in \Pi$ satisfy $\mathrm{sig}(X^{(k)}) = \mathrm{sig}(Y^{(k)})$ for all $k \ge 0$,
  then $X = Y$.
\end{lemma}
...
\begin{lemma}[Antisymmetry of dominance on $\Pi$]
  ...
  For $X, Y \in \Pi$, if $X \le Y$ and $Y \le X$, then $X = Y$.
  Hence $\Pi$ carries a partial order.
\end{lemma}
...
\begin{definition}[Variety $\Lambda$]
  ...
  The variety $\Lambda$ is the filtered variety of all non-polarized chromosomes.
\end{definition}
...
\begin{definition}[Five labeled varieties]
  ...
  \begin{itemize}
    \item $V_0 = \Pi$
    \item $V_1 = \mathrm{Mix}(\Pi, \Lambda)$
    \item $V_2 = \Lambda$
    \item $V_3 = \mathrm{Mix}(\Lambda, \Pi)$
    \item $V_4 = \mathrm{Mix}(\Lambda, \Lambda)$
  \end{itemize}
\end{definition}
...
\begin{lemma}[Difference conditions for $\Pi$]
  ...
  If $X \in \Pi$, then $a_X(k) - b_X(k+1) \le 1$ and $b_X(k) - a_X(k+1) \le 1$.
\end{lemma}
...
\begin{lemma}[Dominance implies componentwise inequality]
  ...
  If $X < Y$ in $\Pi$, then $\sigma_X(k) \le \sigma_Y(k)$ componentwise for all $k$
...
\begin{definition}[Primitive mutation relation]
  ...
  A \emph{primitive mutation} on $\Pi$ is one of the three types above (up to adding a common chromosome $Z$).
\end{definition}
...
\begin{definition}[Step relation]
  ...
  A \emph{step} on $\Pi$ is a primitive mutation plus an arbitrary summand:
...
\begin{theorem}[Existence of mutations]
  ...
  For any $X, Y \in \Pi$ with $\mathrm{rank}(X) = \mathrm{rank}(Y) = n$ and $Y \le X$,
  either $X = Y$ or there exists a step mutation from $X$ towards $Y$:
  there exist $X' \in \Pi$ with $\mathrm{Step}(X, X')$ and $Y \le X' < X$.
\end{theorem}
