...
\begin{remark}[Intended decomposition of the Case A core]
  By analogy with $\mathrm{Pi}$, whose Case A core splits into a single rank-shift
  analysis (type 1) and a double rank-shift analysis (type 2, with four parity
  sub-cases), the mixed Case A core is expected to dispatch on the minimal gene's
  rank parity and type: type 8 plays the role of the same-sign double shift, while
  types 4--7 realize the polarized$\leftrightarrow$nonpolarized single-shift moves.
  Unlike $\mathrm{Pi}$, signatures here are half-integers (the even/$\mathrm{Lambda}$
  part contributes equal components, the odd/$\mathrm{Pi}$ part contributes integers),
  so the window inequalities are stated with increments of $1/2$ rather than $1$; the
  pattern is already used in the disjoint-pair sub-case
  (\texttt{half\_le\_sigma\_diff\_at\_r}).  This decomposition is not yet formalized.
\end{remark}
